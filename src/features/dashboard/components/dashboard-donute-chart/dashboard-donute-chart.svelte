<script>
  ...
  const width = 160 // the outer width of the chart, in pixels
  ...
  const outerRadius = Math.min(width, height) * 0.5 - 35 // the outer radius of the circle, in pixels
  const innerRadius = 70 // the inner radius of the chart, in pixels
  const labelPosition = 1 // the position of the label offset from center
